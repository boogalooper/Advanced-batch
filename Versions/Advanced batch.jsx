...
function main() {
  ...
  function confirmBatch() {
    ...
    function strMsg() {
      ...
      output.push(STR.Counter + CFG.fileList.count - CFG.batch - 1)
...
function Preset() {
  ...
  this.stringToObject = function (arr) {
    var a = arr.split('\v'),
      s = {};
  ...
  function getPresetList() {
    try {
      ...
      for (var i = 0; i < d.count; i++) { p.putObject(d.getKey(i), s2t('object'), AM.objectToDescriptor(new PS.stringToObject(d.getString(d.getKey(i))))) }
      if (PS.saveToFile(d)) app.eraseCustomOptions('advancedBatch')
    } catch (e) { }
  ...
  function getFromFile() {
    var d = new ActionDescriptor(),
      fle = new File(app.preferencesFolder + "/" + STR.Message + ".desc1");
    try {
      if (fle.exists) {
        fle.open('r')
        fle.encoding = 'BINARY'
  ...
  function saveToFile(d) {
    var fle = new File(app.preferencesFolder + "/" + STR.Message + ".desc1");
    try {
      fle.open('w')
      fle.encoding = 'BINARY'
