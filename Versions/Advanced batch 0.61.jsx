...
var localization = "en", // "auto" - use automatic localization, "ru" - russian interface, "en" - english interface
...
function buildWindow(fromEvent) {
  ...
  w.onShow = function (fromPreset) {
    ...
    renew = false;
    if (typeof CFG.options == 'string') (CFG.options = new ActionList()).putObject(gOptions, (new ActionDescriptor())); // затираем старые опции
  ...
  function checkOptionsList(parent) {
    ...
    // ищем последнюю активную строку
    var lastLine = -1
    ...
    // если включен батч-совместимый режим, находим последнюю активированную команду
    ...
    function showErr(mode, labelObj) {
      ...
      // 3 - скрипт не найден
      // 4 - скрипт не может быть использован при переназначении
...
function doLoop() {
  ...
  if (CFG.doneCounter == -1) { // если обработка прошлого документа окончена
    ...
    if (!selectDocument()) {
      // проверить, есть ли документы из очереди на  экране 
      ...
      if (selectDocument()) { // попытаться активировать открытый файл из очереди документов
        ...
        CFG.doneCounter = 0
        CFG.continue = 0
      } // ТУТ БУДЕТ 0 ДОКУМЕНТОВ, НО К ОТКРЫТИЮ УЖЕ НЕ ВЕРНЕТСЯ
    } else {
      result = true
      CFG.doneCounter = 0
      CFG.continue = 0
  ...
  if (CFG.doneCounter != -1) { // если операция не окончена
    if (result) { // если открытие документа было успшеным
      ...
      var toDo = new ActionList() // формируем список активных операций
      ...
      if (CFG.doneCounter <= toDo.count && CFG.doneCounter != -2) { // если текущий номер команды меньше общего количества операций и не установлен ручной запуск
        CFG.doneCounter = CFG.doneCounter < 0 ? 0 : CFG.doneCounter // если установлен ручной запуск -3
        do {
          if (CFG.doneCounter < toDo.count) { // если текущий номер команды меньше количества команд в списке
            ...
            if (cmd.getBoolean(gActive) == true) { // если команда активна
              if (CFG.doneCounter < toDo.count - 1 || Boolean(CFG.override) == false || CFG.saveMode == 0 || CFG.saveMode == 1) { //есди сейчас последняя команда из списка или выключен режим batch или не нужно сохранять файл
                if (mode == 'action') {
                  var cur = CFG.continue; // текущая операция
                  if (AM.isActionAvailable(cmd.getString(gActionSet), cmd.getString(gAction)) == 0) { alert(localize(STR.ErrAction, cmd.getString(gActionSet), cmd.getString(gAction))); CFG.globalErr = true; break; } // если этот экшен есть на панели

                  if (AM.runAction(cmd.getString(gActionSet), cmd.getString(gAction), CFG.continue)) { // если выполнение экшена успешно
                  ...
                  try { $.evalFile(f) } catch (e) { alert(e); CFG.globalErr = true; break; }
                  CFG.doneCounter++
                  CFG.continue = 0
            ...
            } else { alert(localize(STR.ErrBatchMode, STR.Override, cmd.getString(gActionSet), cmd.getString(gAction))); CFG.globalErr = true; break } // если последнее действие деактивировано и мы попали сюда вместо override
