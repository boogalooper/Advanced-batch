...
try {
  fromBridge =  (d = app.playbackParameters).getBoolean(d.getKey(0))
...
function buildWindow(fromEvent) {
  ...
  w.onShow = function (fromPreset) {
    ...
    renew = false;
    if (typeof CFG.options == 'string') (CFG.options = new ActionList()).putObject(gAction, (new ActionDescriptor())); // затираем старые опции
  ...
  function addAction(parent, s) {
    ...
    // ======
    {
      ...
      var chActive = grOpt.add('checkbox', undefined, undefined, { name: 'active' });
      chActive.text = STR.Activate

      var dlSet = grOpt.add("dropdownlist", undefined, undefined, { name: 'set' });
      dlSet.preferredSize.width = 170;

      var dlAtn = grOpt.add("dropdownlist", undefined, undefined, { name: 'action' });
      dlAtn.preferredSize.width = 170;

      var chStop = grOpt.add('checkbox', undefined, undefined, { name: 'stop' });
    ...
    bnAdd.onClick = function () {
      ...
      addAction(parent, (CFG.options.getObjectValue(i)))
    ...
    chActive.onClick = function () {
      var len = parent.children.length
      if (CFG.options.count == len && CFG.override && parent.children[len - 1] == grLine && renew) { checkOptionsList(parent); parent.parent.children[!fromEvent ? 5 : 6].children[0].enabled = bnOkStatus() }
    ...
    chActive.value = s.hasKey(gActive) ? s.getBoolean(gActive) : true
    dlSet.add('item', s.hasKey(gActionSet) ? s.getString(gActionSet) : '')
    dlAtn.add('item', s.hasKey(gAction) ? s.getString(gAction) : '')
    chStop.value = s.hasKey(gStop) ? s.getBoolean(gStop) : false

    // проверяем строку на наличие экшенов
    checkLine()
  ...
  function collectSettings(p) {
    ...
    function readLine(p, list) {
      var d = new ActionDescriptor()
      d.putBoolean(gActive, p.active.value)
      d.putBoolean(gStop, p.stop.value)
      d.putString(gActionSet, p.set.selection)
      d.putString(gAction, p.action.selection)
      l.putObject(gAction, d)
    }
  }
...
function doLoop() {
  ...
  if (CFG.doneCounter != -1) { // если операция не окончена
    if (result) { // если открытие документа было успшеным
      ...
      if (CFG.doneCounter <= CFG.options.count && CFG.doneCounter != -2) { // если текущий номер команды меньше общего количества операций и не установлен ручной запуск
        CFG.doneCounter = CFG.doneCounter < 0 ? 0 : CFG.doneCounter // если установлен ручной запуск -3
        do {
          if (CFG.doneCounter < CFG.options.count) { // если текущий номер команды меньше количества команд в списке
            var cmd = CFG.options.getObjectValue(CFG.doneCounter),
              set = cmd.getString(gActionSet),
              atn = cmd.getString(gAction);
            if (cmd.getBoolean(gActive) == true) { // если команда активна
              if (AM.isActionAvailable(set, atn) == 0) { alert(localize(STR.ErrAction, set, atn)); CFG.globalErr = true; break; } // если этот экшен есть на панели

              if (CFG.doneCounter < CFG.options.count - 1 || Boolean(CFG.override) == false || CFG.saveMode == 0 || CFG.saveMode == 1) { //есди сейчас последняя команда из списка или выключен режим batch или не нужно сохранять файл
                var cur = CFG.continue // текущая операция + номер команды в виде текста
                var isDone = AM.runAction(set, atn, CFG.continue) // выполняем экшен

                if (isDone == true) { // если выполнение экшена успешно
                  CFG.doneCounter++ // следующая операция в списке
                  if (cmd.getBoolean(gStop)) break; // если установлен стоп после команды, то выходим не завершая цикл
                }
                else {
                  if (cur == CFG.continue && CFG.continue != 0) CFG.continue = -1 // если операция встала на том же месте, где и при прошлом запуске, считаем что она выполнена
...
function Config() {
  ...
  this.jpgQuality = 12
  this.override = false;
  (this.options = new ActionList()).putObject(gAction, (new ActionDescriptor()));
  this.batch = -1
  this.doneCounter = -1
  this.continue = 0
...
function Preset() {
  ...
  function stringToObject(arr) {
    ...
    function actionLineToDesc(s) {
      ...
      for (var i = 0; i < len; i++) {
        ...
        d.putBoolean(gStop, key[4] == "1" ? true : false)
  ...
  this.settingsToPreset = function (s) {
    ...
    d.putObject(gAction, gObject, AM.objectToDescriptor(p))
    return d.getObjectValue(gAction)
  }
