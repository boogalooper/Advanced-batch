...
var dlSave_array = [strNone,strSAC,"сохранить как... и закрыть", strSTF]; 
...
chBatchMode.onClick = function () 
{
  ...
  if (settings.saveMode != 0) pnAtn.children[pnAtn.children.length-1].children[1].children[3].visible = !settings.override
  checkPresetIntegrity()
}
...
function addAction (parent, s)
{
    ...
    bnAdd.onClick = function ()
    {
      ...
      if (settings.saveMode != 0) parent.children[i].children[1].children[3].visible = !settings.override
...
function doBatch (skipOpened)
{
 ...
 // alert ("входящий номер операции " + settings.doneCounter + "\nвходящий номер команды " + settings.continue)
      ...
      do
      {
        if (getDocumentID () || settings.saveMode == 0)  //если ранее открытый документ есть на экране или установлен режим "не сохранять"
        {
          if (settings.doneCounter<todo.length)
          {
            ...
            {
             // alert ("выполняем операцию " + settings.doneCounter + "\nномер команды " + settings.continue)
              if (settings.doneCounter<todo.length-1 || Boolean(settings.override) == false || settings.saveMode == 0)
                {
               ...
               //   alert ("сохраняем в режиме batch")      
                ...
                settings.doneCounter = -1
                if (settings.saveMode!=0) doBatch ()
                break;
              }
            } else {settings.doneCounter++} // если строка неактивна, пропустить
          } 
          else 
          {
       //     alert ("сохраняем при помощи save")
            if (!settings.override || settings.saveMode == 0)
            {
              if (settings.saveMode == 1 || settings.saveMode == 2)
              {
            ...
            //      alert ("сохранение не требуется, ставим флаг успешного выполнения команды!")
                ...
                case 1:
                  ...
                  break;
                case 2:
                  var fol = Boolean (settings.keepStruct) ? rebuildPath(pth, settings.lastSavePath) : Folder (settings.lastSavePath)
                  if (!fol.exists) {alert (strErrFolder); settings.globalErr = true; break}
...
{
    var desc = new ActionDescriptor();
    desc.putEnumerated( gSave, gYesNo, gNo)
